...
if __name__ == "__main__":
    ...
    exp = 'bmhad_KD' #Assign an experiment id
    dataset = 'bmhad'
    mocap_frames = 600
    acc_frames = 256
    num_joints =31
    ...
    student_model = ActTransformerAcc(adepth = 3,num_classes=num_classes, acc_frames= acc_frames, num_joints =num_joints,has_features=False, num_heads=4, acc_embed = 32)

    teacher_model.to(device=device)
    student_model.to(device=device)

    teacher_model.load_state_dict(torch.load('exps/bmhad/bhmadmmd4h8_woKD_norandom_revised.pt', map_location=torch.device('cpu')))
    #Optimizer
        ...
        with tqdm(total  = len(training_generator), desc = f'Epoch {epoch}/{num_epochs}',ncols = 128) as pbar:
            ...
            with torch.no_grad():
                for inputs,targets in validation_generator:
                    ...
                    out, student_logits,predictions = student_model(inputs.float())
                    teacher_out, teacher_logits, teacher_pred = teacher_model(inputs.float())
                    loss_score = nn.CrossEntropyLoss()(predictions, targets)
            ...
            if best_accuracy < val_accuracy:
                    best_accuracy = val_accuracy
                    #need to add arguements here also for different experiments
                    torch.save(student_model.state_dict(),PATH+exp+'.pt'); 
                    print("Check point "+PATH+exp+'.pt'+ ' Saved!')
